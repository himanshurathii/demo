...
export default function ManageCasesModal({
  open,
  setOpen,
  onManageCasesCallback = () => {},
}) {
  ...
  return (
    <Dialog
      onClose={handleClose}
      aria-labelledby="manage-cases-dialog-title"
      open={open}
      maxWidth="xs"
      fullWidth={true}
    >
      <DialogTitle
        id="manage-cases-dialog-title"
        onClose={handleClose}
      >
        <Box
          display="flex"
          ...
          alignItems="center"
        >
          <Typography variant="h6">Manage Cases</Typography>
          <IconButton
            color="primary"
            aria-label="close"
            onClick={() => {
